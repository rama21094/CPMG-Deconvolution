"""Generate ChemEx-style intensity files using ChemEx's OWN cpmg_15n_ip pulse
sequence (CPMGSimulator.simulate_cpmg_chemex_sequence), not the home simulator's
idealized train. This removes the pulse-sequence/timing mismatch entirely from
the home-vs-ChemEx fit comparison, so any residual bias in a ChemEx fit against
this data is attributable ONLY to ChemEx's reduced 6x6 basis (missing proton
bath, CSA-DD cross-correlation, J-coupling) -- not to sequence conventions.

Mirrors cpmg_ml/generate_chemex_sweep.py's grid, fixed parameters, and file
format exactly (so results are directly comparable to the earlier home-sequence
batch), except:
  - ncyc is ChemEx's own convention (pulses per half-train) and is written to
    the .out files directly, with no /2 relabeling.
  - time_t2 passed to the sequence equals t_relax with no separate correction
    needed (both already 0.04 s).
"""
...
DEFAULT_OUT_DIR = Path("data/chemex_j0")
DEFAULT_T_RELAX = 0.04
DEFAULT_TIME_EQUIL = 2.0e-3

# ChemEx's own ncyc convention: pulses per half-train (real total pulse count
# is 2*ncyc + 1, including the central compensating 180). Written to the .out
# files directly -- no relabeling needed, unlike the home-sequence generator.
DEFAULT_NCYC_GRID = np.arange(1, 41, dtype=np.int32)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Generate ChemEx-style intensity files using ChemEx's own "
            "cpmg_15n_ip pulse sequence, propagated through the full 32x32 "
            "Allard physics."
        )
    )
...
def main() -> None:
    ...
    for field_index, b0_mhz in enumerate(b0_values):
        for case_id, (k_ex, p_b, dw_n) in enumerate(parameter_grid):
            ...
            profile = sim.simulate_cpmg_chemex_sequence(
                params,
                ncyc_range=DEFAULT_NCYC_GRID,
                time_t2=DEFAULT_T_RELAX,
                time_equil=DEFAULT_TIME_EQUIL,
            )
    ...
    print("ChemEx-matched-sequence sweep complete.")
