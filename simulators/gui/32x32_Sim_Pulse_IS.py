...
class TwoSpinExchangeCPMGSimulator:
    """Exact 32x32 CPMG simulation with Two-Site Chemical Exchange and J-coupling"""
    
    def __init__(self, R2_I=12.0, R2_S=15.0, J=90.0, 
                 kex=1000.0, pB=0.05, dw_I_ppm=2.0, dw_S_ppm=0.0, 
                 B0=600.0, timeT2=40.0, ncycMax=30, pulse_S=False):
    ...
    def get_ideal_pi_pulse_32(self):
        ...
        if self.pulse_S:
            # X-axis pulse on BOTH I and S spins
            flip_indices = [2, 3, 5, 6, 7, 9, 12, 13]
        else:
            # X-axis pulse on Spin I ONLY (Meiboom-Gill locked to Ix)
    ...
    def calculate_R2eff(self, ncyc):
        ...
        P_x = self.get_ideal_pi_pulse_32()
        
        cycle_prop = E_tau @ P_x @ E_2tau @ P_x @ E_tau
        total_prop = np.linalg.matrix_power(cycle_prop, ncyc)
        
        v_0 = np.zeros(32, dtype=complex)
        v_0[0], v_0[1] = self.pA, self.pA
        v_0[16], v_0[17] = self.pB, self.pB
...
class Exchange32GUI:
    def __init__(self, master):
        self.master = master
        master.title("32x32 Chemical Exchange CPMG Simulator")
        ...
        self.show_comparison = tk.BooleanVar(value=True)
        self.pulse_S_spin = tk.BooleanVar(value=False)
    ...
    def setup_gui(self):
        ...
        ttk.Separator(control_frame, orient='horizontal').grid(row=row, column=0, columnspan=3, sticky=(tk.W, tk.E), pady=10); row += 1
        
        ttk.Checkbutton(control_frame, text="Apply 180° Pulse to Spin S", variable=self.pulse_S_spin, command=self.update_plot).grid(row=row, column=0, columnspan=3, sticky=tk.W); row += 1
        ttk.Checkbutton(control_frame, text="Show Comparison (J=0)", variable=self.show_comparison, command=self.update_plot).grid(row=row, column=0, columnspan=3, sticky=tk.W); row += 1
    ...
    def update_plot(self, *args):
        ...
        # Primary Simulation (With J-Coupling)
        sim = TwoSpinExchangeCPMGSimulator(
            ...
            pulse_S=self.pulse_S_spin.get()
        )
        vcpmg, R2eff = sim.generate_profile()
        self.ax.plot(vcpmg, R2eff, 'o-', color='darkblue', linewidth=2, markersize=6, label=f'J = {p["J"]:.1f} Hz')
        
        # Reference Simulation (No J-Coupling)
        if self.show_comparison.get():
            sim_no_J = TwoSpinExchangeCPMGSimulator(
                kex=p['kex'], pB=p['pB'], dw_I_ppm=p['dw_I'], dw_S_ppm=p['dw_S'],
                R2_I=p['R2_I'], R2_S=p['R2_S'], J=0.0, 
                B0=p['B0'], timeT2=p['timeT2'], ncycMax=int(p['ncycMax']),
                pulse_S=self.pulse_S_spin.get()
            )
            vcpmg_noJ, R2eff_noJ = sim_no_J.generate_profile()
            self.ax.plot(vcpmg_noJ, R2eff_noJ, 's--', color='orange', linewidth=2, markersize=5, label='J = 0 Hz')
