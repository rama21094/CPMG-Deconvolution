...
class TwoSpinCPMGSimulator:
    ...
    def __init__(self, l_I=10.0, l_S=10.0, r_I=2.0, r_S=2.0, 
        ...
        # Frequencies and coupling (rad/s)
        self.V_I = offset_I_ppm * B0 * 2.0 * np.pi  
        self.V_S = offset_S_ppm * B0 * 2.0 * np.pi  
        self.piJ = np.pi * J        
    ...
    def build_gamma_free(self):
        ...
        # 1. FIX: Must be complex to prevent scipy expm overflow
        ...
        # 2. FIX: Added the missing -self.V_I term here in G[14, 12]
        G[14, 11] = -self.m_mq; G[14, 12] = -self.V_I; G[14, 13] = -self.V_S; G[14, 14] = self.l_mq
    ...
    def calculate_R2eff(self, ncyc):
        ...
        # Clean calculation: no clipping or suppressing warnings needed anymore
        ...
        # Start pure Ix (Must be complex)
        v_0 = np.zeros(16, dtype=complex) 
        v_0[0] = 1.0; v_0[1] = 1.0
        
        v_final = total_prop @ v_0
        
        # 4. FIX: Extract the real component at the very end
        Ix_final = np.real(v_final[1])
        
        if Ix_final <= 0: 
            return self.l_I, vcpmg
            
        R2eff = -(1.0 / self.timeT2) * np.log(Ix_final / np.real(v_0[1]))
