# -*- coding: utf-8 -*-
"""
Created on Mon Oct 13 08:43:00 2025
...
class CPMGSimulatorFinitePulse:
    ...
    def calculate_R2eff(self, ncyc, add_noise=False, noise_level=0.02):
        """
        Calculate R2eff with finite pulse widths
        
        CPMG sequence timing:
        90°x - τ - [180°x(τpw) - τ]n
        
        Where τ is adjusted to account for pulse width:
        τ_effective = τ_delay - τpw/2
        ...
        """
        # Pulse width in seconds
        tau_pw = self.pulse_width / 1e6  # convert μs to s
        
        # Total time per cycle (4 tau periods between pulses)
        # But we need to account for pulse widths
        time_per_cycle = (self.timeT2 / 1000.0) / ncyc
        
        if self.use_finite_pulses:
            # Actual delay between pulse centers
            tau = time_per_cycle / 4.0
            # Effective free evolution time (excluding pulse width)
            tau_eff = tau - tau_pw / 2.0
            
            if tau_eff <= 0:
                # Pulse width exceeds delay - not physical
        ...
        # CPMG frequency based on effective delays
        vcpmg = 1.0 / (4.0 * tau)
        ...
        for k in range(ncyc):
            ...
            # 180° pulse
            if self.use_finite_pulses:
                state = self.apply_finite_180_pulse(state, tau_pw)
            else:
...
class CPMGSimulatorGUI:
    ...
    def create_slider_in_frame(self, parent, row, label, var_name, min_val, max_val, step, scale=1):
        """Create slider in a sub-frame (like the pulse parameters frame)"""
        var = self.params[var_name]
